...
TH3D *hPosKaonPtEtaPhi_gen = new TH3D("hPosKaonPtEtaPhi_gen", "hPosKaonPtEtaPhi_gen; p_{T} [GeV/c]; #eta; #phi", nbin_pt, min_pt, max_pt, nbin_eta, min_eta, max_eta, nbin_phi, min_phi, max_phi);
TH3D *hNegKaonPtEtaPhi_gen = new TH3D("hNegKaonPtEtaPhi_gen", "hNegKaonPtEtaPhi_gen; p_{T} [GeV/c]; #eta; #phi", nbin_pt, min_pt, max_pt, nbin_eta, min_eta, max_eta, nbin_phi, min_phi, max_phi);
TH3D *hKaonPtEtaPhi_gen = new TH3D("hKaonPtEtaPhi_gen", "hKaonPtEtaPhi_gen; p_{T} [GeV/c]; #eta; #phi", nbin_pt, min_pt, max_pt, nbin_eta, min_eta, max_eta, nbin_phi, min_phi, max_phi);

TH3D *hPtRapMass_gen = new TH3D("hPtRapMass_gen", "hPtRapMass_gen; p_{T} [GeV/c]; Rapidity; Mass [GeV/c^{2}]", nbin_pt, min_pt, max_pt, nbin_rap, min_rap, max_rap, nbin_mass, min_mass, max_mass);

TH3D *hPtRapMass_gen_rec = new TH3D("hPtRapMass_gen_rec", "hPtRapMass_gen_rec; p_{T} [GeV/c]; Rapidity; Mass [GeV/c^{2}]", nbin_pt, min_pt, max_pt, nbin_rap, min_rap, max_rap, nbin_mass, min_mass, max_mass);
//# Functions ###############################################################################################################
...
void plot_hist_projection(TH3D *h3)
{
	...
	gStyle->SetOptStat(0);
	h1_x->Draw("E");
	c->SaveAs(Form("outFigures/mcPhi/%s_x.png", h3->GetName()));

	h1_y->Draw("E");
	c->SaveAs(Form("outFigures/mcPhi/%s_y.png", h3->GetName()));
	...
	c->SaveAs(Form("outFigures/mcPhi/%s_z.png", h3->GetName()));

	delete c;
}

//# Main ####################################################################################################################
void VCTree_trigEff_Phi_mc()
{
	...
	// TString inputFile = "inFiles/2023/diKa_ana_mc_1M_tight.root";
	// TString inputFile = "inFiles/2023/diKa_ana_mc_1M_loose.root";
	TString inputFile = "inFiles/2023/diKa_ana_mc_1M_noCut.root";
	...
	TTreeReaderValue<std::vector<Float_t>>		gen_y(treeReader,		"gen_y");
	TTreeReaderValue<vector<vector<unsigned short> >>		gen_dauIdx(treeReader,	"gen_dauIdx");
	TTreeReaderValue<vector<vector<unsigned int> >>		gen_candIdx(treeReader,	"gen_candIdx");
	...
	while (treeReader.Next()) {
		...
		Bool_t passEvtSel = evtSel->at(0);	// Vertex + HF<8
		...
		bool   passHLTtrig = passHLT->at(3) || passHLT->at(6);
		

		//# Gen START--------------------------------------------------------------------------------------------
		for (int icand_gen = 0; icand_gen < gen_mass->size(); icand_gen++)
		{
			// * catch phi rec
			if (IsPhi(gen_pdgId->at(icand_gen)))
			{
				double pt_gen   = gen_pT->at(icand_gen);
				double eta_gen  =	gen_eta->at(icand_gen);
				double phi_gen  =	gen_phi->at(icand_gen);
				double mass_gen =	gen_mass->at(icand_gen);
				double y_gen    =	gen_y->at(icand_gen);

				hPtRapMass_gen->Fill(pt_gen,y_gen, mass_gen);

				//* Get the daughter kaons index
				auto daughters = gen_dauIdx->at(icand_gen);
				for ( int idau = 0; idau < daughters.size(); idau++)
				{
					int change = gen_pdgId->at(daughters[idau]);
					int pdgId = gen_pdgId->at(daughters[idau]);
					double pt = gen_pT->at(daughters[idau]);
					double eta = gen_eta->at(daughters[idau]);
					double phi = gen_phi->at(daughters[idau]);

					// cout << "pdgId: " << pdgId << " pt: " << pt << " eta: " << eta << " phi: " << phi << endl;

					if (change > 0)
					{
						hPosKaonPtEtaPhi_gen->Fill(pt, eta, phi);
					}
					else if (change < 0)
					{
						hNegKaonPtEtaPhi_gen->Fill(pt, eta, phi);
					}
				}

				//* Find the reco 
				auto candIdx = gen_candIdx->at(icand_gen);
				for (int irec = 0; irec < candIdx.size(); irec++)
				{
					int idx = candIdx.at(irec);
					if (idx >= cand_mass->size()) continue;
					double pt_rec   = cand_pT->at(idx);
					double eta_rec  =	cand_eta->at(idx);
					double phi_rec  =	cand_phi->at(idx);
					double mass_rec =	cand_mass->at(idx);
					double y_rec    =	cand_y->at(idx);

					hPtRapMass_gen_rec->Fill(pt_rec,y_rec, mass_rec);
				}
			}
		}
		...
		for (int irec = 0; irec < cand_mass->size(); irec++)
		{
			...
			if (IsPhi(cand_pdgId->at(irec)))
			{
				...
				// if (pt_rec > 0.2) continue;
				// if (abs(y_rec) < 1.4 || abs(y_rec) > 2.4) continue;
	...
	// plot_hist2D_compare(hPosKaonPtEtaPhi, hNegKaonPtEtaPhi, "xy", "hPosNegKaonPtEta");
	// plot_hist2D_compare(hPosKaonPtEtaPhi_gen, hNegKaonPtEtaPhi_gen, "xy", "hPosNegKaonPtEta_gen");

	plot_hist1D_compare(hPtRapMass, hPtRapMass_gen, "x", "mcPhi/hgen_reco_Pt");
	plot_hist1D_compare(hPtRapMass, hPtRapMass_gen, "y", "mcPhi/hgen_reco_Rap");
	plot_hist1D_compare(hPtRapMass, hPtRapMass_gen, "z", "mcPhi/hgen_reco_Mass");

	hKaonPtEtaPhi->Add(hPosKaonPtEtaPhi, hNegKaonPtEtaPhi);
	hKaonPtEtaPhi_gen->Add(hPosKaonPtEtaPhi_gen, hNegKaonPtEtaPhi_gen);

	plot_hist1D_compare(hKaonPtEtaPhi, hKaonPtEtaPhi_gen, "x", "mcPhi/hgen_reco_Kaon_Pt");
	plot_hist1D_compare(hKaonPtEtaPhi, hKaonPtEtaPhi_gen, "y", "mcPhi/hgen_reco_Kaon_Eta");
	plot_hist1D_compare(hKaonPtEtaPhi, hKaonPtEtaPhi_gen, "z", "mcPhi/hgen_reco_Kaon_Phi");

	plot_hist2D(hKaonPtEtaPhi_gen, "xy", "mcPhi/hgen_Kaon_PtEta");
	plot_hist2D(hKaonPtEtaPhi, "xy", "mcPhi/hreco_Kaon_PtEta");

	plot_hist_projection(hKaonPtEtaPhi);
	plot_hist_projection(hKaonPtEtaPhi_gen);

	plot_hist_projection(hPtRapMass);
	plot_hist_projection(hPtRapMass_gen);
	plot_hist_projection(hPtRapMass_gen_rec);
}